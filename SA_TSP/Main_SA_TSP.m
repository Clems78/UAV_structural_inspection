...
while T0 > Tend
    ...
    T0 = q * T0; %降温
     %% 显示此代信息
    fprintf('Iteration = %d, Min Distance = %.2f m  \n',count,BestObjByIter(count)/1000) %输出当前迭代信息
end

%% 找出历史最短距离和对应路径
mindisever = BestObjByIter(count)/1000; %找出历史最优目标函数值
...
%     alt_changes_sa = alt_changes_sa + abs(waypointsOrdered(i+1, 3) - waypointsOrdered(i, 3));
% end
% disp(['Overall altitude changes: ', num2str(round(alt_changes_sa/1e3, 2)), ' m']);
% end
